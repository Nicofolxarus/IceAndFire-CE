...
public abstract class MultipartPartEntity extends Entity implements Tameable {
    ...
    private static final TrackedData<Float> SCALE_HEIGHT = DataTracker.registerData(MultipartPartEntity.class, TrackedDataHandlerRegistry.FLOAT);
    private static final TrackedData<Float> PART_YAW = DataTracker.registerData(MultipartPartEntity.class, TrackedDataHandlerRegistry.FLOAT);
    ...
    protected MultipartPartEntity(EntityType<?> t, World world) {
        super(t, world);
    ...
    public void collideWithNearbyEntities() {
        ...
        Entity parent = this.getParent();
        if (parent != null) {
            entities.stream().filter(entity -> entity != parent && !sharesRider(parent, entity) && !(entity instanceof MultipartPartEntity) && entity.isPushable()).forEach(entity -> entity.pushAwayFrom(parent));
        }
    }
