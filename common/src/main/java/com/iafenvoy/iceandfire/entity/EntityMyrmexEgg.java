    ...
    public void tick() {
        ...
        if (this.getMyrmexAge() > IafCommonConfig.INSTANCE.myrmex.eggTicks.getValue()) {
            ...
            EntityMyrmexBase myrmex = switch (this.getMyrmexCaste()) {
                case 1 -> new EntityMyrmexSoldier(IafEntities.MYRMEX_SOLDIER.get(), this.getWorld());
                case 2 -> new EntityMyrmexRoyal(IafEntities.MYRMEX_ROYAL.get(), this.getWorld());
                case 3 -> new EntityMyrmexSentinel(IafEntities.MYRMEX_SENTINEL.get(), this.getWorld());
                case 4 -> new EntityMyrmexQueen(IafEntities.MYRMEX_QUEEN.get(), this.getWorld());
                default -> new EntityMyrmexWorker(IafEntities.MYRMEX_WORKER.get(), this.getWorld());
            };
