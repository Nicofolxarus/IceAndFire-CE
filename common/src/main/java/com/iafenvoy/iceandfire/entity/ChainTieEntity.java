...
public class ChainTieEntity extends AbstractDecorationEntity {
    public ChainTieEntity(EntityType<? extends AbstractDecorationEntity> type, World worldIn) {
        super(type, worldIn);
    }

    public ChainTieEntity(EntityType<? extends AbstractDecorationEntity> type, World worldIn, BlockPos hangingPositionIn) {
    ...
    public static ChainTieEntity createTie(World worldIn, BlockPos fence) {
        ChainTieEntity chainTieEntity = new ChainTieEntity(IafEntities.CHAIN_TIE.get(), worldIn, fence);
        worldIn.spawnEntity(chainTieEntity);
        chainTieEntity.onPlace();
        return chainTieEntity;
    }

    public static ChainTieEntity getKnotForPosition(World worldIn, BlockPos pos) {
        int i = pos.getX();
        int j = pos.getY();
        int k = pos.getZ();

        for (ChainTieEntity entityleashknot : worldIn.getNonSpectatingEntities(ChainTieEntity.class, new Box(i - 1.0D, j - 1.0D, k - 1.0D, i + 1.0D, j + 1.0D, k + 1.0D)))
