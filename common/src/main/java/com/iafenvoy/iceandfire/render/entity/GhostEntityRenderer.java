...
public class GhostEntityRenderer extends MobEntityRenderer<GhostEntity, GhostModel> {
    ...
    @Override
    public void render(GhostEntity entityIn, float entityYaw, float partialTicks, MatrixStack matrixStackIn, VertexConsumerProvider bufferIn, int packedLightIn) {
        this.shadowRadius = 0;
        matrixStackIn.push();
        ...
        if (!entityIn.isSpectator())
            for (FeatureRenderer<GhostEntity, GhostModel> layerrenderer : this.features)
    ...
    @Override
    public void scale(GhostEntity LivingEntityIn, MatrixStack stack, float partialTickTime) {
    }
