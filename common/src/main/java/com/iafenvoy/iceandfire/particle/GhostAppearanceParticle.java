...
public class GhostAppearanceParticle extends Particle {
    ...
    public GhostAppearanceParticle(ClientWorld worldIn, double xCoordIn, double yCoordIn, double zCoordIn, int ghost) {
    ...
    public void buildGeometry(VertexConsumer buffer, Camera renderInfo, float partialTicks) {
        ...
        if (entity instanceof GhostEntity ghostEntity && MinecraftClient.getInstance().options.getPerspective() == Perspective.FIRST_PERSON) {
            ...
            VertexConsumer ivertexbuilder = irendertypebuffer$impl.getBuffer(IafRenderLayers.getGhost(GhostEntityRenderer.getGhostOverlayForType(ghostEntity.getColor())));
