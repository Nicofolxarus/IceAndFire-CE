    ...
    protected void drawBackground(DrawContext ms, float partialTicks, int mouseX, int mouseY) {
        ...
        for (int i1 = 0; i1 < 3; ++i1) {
            ...
            if (l1 == -1)
                ...
                int j2 = 6839882;
                if (this.handler.getSlot(0).getStack().getItem() == IafItems.BESTIARY.get()) { // Forge: render buttons as disabled when enchantable but enchantability not met on lower levels
                    int k2 = mouseX - (i + 60);
